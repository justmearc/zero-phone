...
class App():
    ...
    def load_logos(self):
        ...
        for i in range(0, 4):
            logos['surfaces'].append(pygame.image.load('/home/pi/zero-phone/' + self.app_order[i] + '.png'))
            logos['rects'].append(logos['surfaces'][i].get_rect())

        for i in range(0, 4):
            logos['rects'][i].centery = -33
            logos['rects'][i].centerx = 26 + 52 * i
    ...
    def check(self, event):
        if event.type == MOUSEBUTTONDOWN:
            ...
            if event.pos[1] < 20 and self.opened == False:
                self.open_apps = True
                self.close_apps = False
                self.opened = True
            #Check for touch to close app bar
            if event.pos[1] > 85 and self.opened:
                self.open_apps = False
                self.close_apps = True
                self.opened = False
            #Check for touch to open an app
            if self.opened and self.logos['rects'][0].centery == 33:
                if event.pos[0] < 52 and event.pos[1] < 65:
                    self.app_to_open = 0
                if event.pos[0] < 104 and event.pos[0] > 52 and event.pos[1] < 65:
                    self.app_to_open = 1
                if event.pos[0] < 156 and event.pos[0] > 104 and event.pos[1] < 65:
                    self.app_to_open = 2
                if event.pos[0] < 208 and event.pos[0] > 156 and event.pos[1] < 65:
                    self.app_to_open = 3
